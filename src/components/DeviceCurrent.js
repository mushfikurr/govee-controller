...
export function DeviceCurrent(device) {
  ...
  const [color, updateColor] = useState(
    JSON.stringify({ color: { r: 0, b: 0, g: 0 } })
  );
  ...
  useEffect(() => {
    setLoading(true);
    getAPIDeviceState(device).then((response) => {
      ...
      getAPIDeviceState(device)
        .then((response) => {
          if (hasMounted) {
            ...
            const deviceData = response.data.data;
            if (
              Object.values(deviceData.properties[indexToBeChecked]).includes(
                expectedValue[1]
              )
            ) {
              console.log("Matches!");
  ...
  const handlePower = (_) => {
    if (!isOnCooldown) updatePowerState(newPowerState);
  };
  ...
  useEffect(() => {
    if (hasMounted && !isOnCooldown) {
      ...
      dispatchAPICommand(command, device).then((response) => {
  ...
  const [currentProperty, setCurrentProperty] = useState("Brightness");
  const handlePropertyClick = (propertyName) => {
    if (propertyName != currentProperty) {
